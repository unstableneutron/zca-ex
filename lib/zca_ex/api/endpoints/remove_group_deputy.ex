defmodule ZcaEx.Api.Endpoints.RemoveGroupDeputy do
  @moduledoc "Remove group deputy (admin)"
  ...
  @doc """
  Remove deputy (admin) from a group.

  ## Parameters
    - group_id: Group ID
    - member_id: Single member ID or list of member IDs to remove as deputies
  ...
  @doc "Build URL for remove group deputy endpoint with encrypted params"
  @spec build_url(Session.t(), String.t()) :: String.t()
  def build_url(session, encrypted_params) do
    base_url = get_service_url(session, :group) <> "/api/group/admins/remove"
