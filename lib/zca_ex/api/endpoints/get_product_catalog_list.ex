defmodule ZcaEx.Api.Endpoints.GetProductCatalogList do
  ...
  defp validate_version_catalog(_),
    do:
      {:error,
       Error.new(:api, "version_catalog must be a non-negative integer", code: :invalid_input)}
  ...
  defp validate_last_product_id(_),
    do:
      {:error,
       Error.new(:api, "last_product_id must be an integer or non-empty string",
         code: :invalid_input
       )}
