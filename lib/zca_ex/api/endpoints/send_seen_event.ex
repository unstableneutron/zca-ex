defmodule ZcaEx.Api.Endpoints.SendSeenEvent do
  @moduledoc "Mark messages as seen"
  ...
  ## Parameters
    - `messages` - A single message or list of messages to mark as seen
  ...
  defp validate_same_thread(messages, thread_id, is_group) do
    all_same? =
      Enum.all?(messages, fn msg ->
        current_thread_id = if is_group, do: msg.id_to, else: msg.uid_from
    ...
    if all_same? do
      ...
      {:error, %ZcaEx.Error{message: "All messages must belong to the same thread", code: nil}}
    end
  end
