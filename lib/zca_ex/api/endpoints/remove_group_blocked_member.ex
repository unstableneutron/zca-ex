defmodule ZcaEx.Api.Endpoints.RemoveGroupBlockedMember do
  @moduledoc "Remove member(s) from group block list"
  ...
  @doc """
  Remove member(s) from group block list.

  ## Parameters
    - group_id: Group ID
    - member_id: Single member ID or list of member IDs to unblock
  ...
  @doc "Build URL for remove group blocked member endpoint with encrypted params"
  @spec build_url(Session.t(), String.t()) :: String.t()
  def build_url(session, encrypted_params) do
    base_url = get_service_url(session, :group) <> "/api/group/blockedmems/remove"
  ...
  defp validate_member_ids([]), do: {:error, %Error{message: "member_id cannot be empty", code: :invalid_input}}
