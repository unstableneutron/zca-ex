defmodule ZcaEx.Api.Endpoints.UpdateAutoReply do
  @moduledoc """
  ...
  def update(id, content, enabled?, start_time, end_time, scope, uids, session, credentials) do
    with :ok <- validate_id(id),
      ...
      params = build_params(id, content, enabled?, start_time, end_time, scope, uids, credentials)
  ...
  @doc false
  def build_params(id, content, enabled?, start_time, end_time, scope, uids, credentials) do
    result_uids = if scope in [2, 3], do: safe_normalize_uids(uids), else: []
  ...
  def build_url(service_url, session) do
    base_url = service_url <> "/api/autoreply/update"
