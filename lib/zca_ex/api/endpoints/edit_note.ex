defmodule ZcaEx.Api.Endpoints.EditNote do
  @moduledoc "Edit a note (topic) in a group board"
  ...
  @doc """
  Edit a note in a group.

  ## Parameters
    - group_id: Group ID (non-empty string)
    - topic_id: Topic/Note ID to edit (non-empty string)
    - title: New note title (non-empty string)
    - pin?: Whether to pin the note (default: false, meaning pinAct: 2 = unpin)
  ...
  def edit(_group_id, topic_id, _title, _pin?, _session, _credentials)
      when not is_binary(topic_id) or topic_id == "" do
    {:error, Error.new(:api, "topic_id must be a non-empty string", code: :invalid_input)}
  end

  def edit(_group_id, _topic_id, title, _pin?, _session, _credentials)
  ...
  def edit(group_id, topic_id, title, pin?, session, credentials) do
    with {:ok, params} <- build_params(group_id, topic_id, title, pin?, credentials),
  ...
  @doc "Build URL for edit note endpoint"
  @spec build_url(Session.t()) :: String.t()
  def build_url(session) do
    base_url = get_service_url(session) <> "/api/board/topic/updatev2"
