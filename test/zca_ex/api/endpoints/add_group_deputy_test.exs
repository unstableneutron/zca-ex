defmodule ZcaEx.Api.Endpoints.AddGroupDeputyTest do
  use ExUnit.Case, async: true

  alias ZcaEx.Api.Endpoints.AddGroupDeputy
  ...
  describe "build_params/3" do
    test "builds correct params with single member_id" do
      params = AddGroupDeputy.build_params("group123", ["member456"], "test-imei")
    ...
    test "builds correct params with list of member_ids" do
      ...
      params = AddGroupDeputy.build_params("group123", member_ids, "test-imei")
  ...
  describe "normalize_member_ids/1" do
    test "converts single string to list" do
      assert AddGroupDeputy.normalize_member_ids("member123") == ["member123"]
    end

    test "keeps list as is" do
      members = ["member1", "member2"]
      assert AddGroupDeputy.normalize_member_ids(members) == members
    end
  end

  describe "build_base_url/1" do
    test "builds correct base URL", %{session: session} do
      url = AddGroupDeputy.build_base_url(session)

      assert url =~ "https://groupchat.zalo.me/api/group/admins/add"
  ...
  describe "build_url/2" do
    test "builds URL with encrypted params", %{session: session} do
      ...
      url = AddGroupDeputy.build_url(session, encrypted)
  ...
  describe "call/4 validation" do
    test "returns error for empty member_id list", %{session: session, credentials: credentials} do
      result = AddGroupDeputy.call("group123", [], session, credentials)
    ...
    test "raises for missing service URL", %{session: session, credentials: credentials} do
      ...
      assert_raise RuntimeError, ~r/Service URL not found/, fn ->
        AddGroupDeputy.call("group123", "member456", session_no_service, credentials)
      end
    end
  end
end
