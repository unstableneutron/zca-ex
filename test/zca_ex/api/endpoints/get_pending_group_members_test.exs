defmodule ZcaEx.Api.Endpoints.GetPendingGroupMembersTest do
  use ExUnit.Case, async: true

  alias ZcaEx.Api.Endpoints.GetPendingGroupMembers
  ...
  describe "build_params/2" do
    test "builds params with group_id and imei", %{credentials: credentials} do
      params = GetPendingGroupMembers.build_params("group123", credentials)
  ...
  describe "build_base_url/1" do
    test "builds correct base URL with session params", %{session: session} do
      url = GetPendingGroupMembers.build_base_url(session)

      assert url =~ "https://groupchat.zalo.me/api/group/pending-mems/list"
  ...
  describe "build_url/2" do
    test "builds correct URL with encrypted params", %{session: session} do
      ...
      url = GetPendingGroupMembers.build_url(session, encrypted_params)
  ...
  describe "call/3 input handling" do
    test "returns error when group_id is empty", %{session: session, credentials: credentials} do
      assert {:error, %ZcaEx.Error{message: "group_id cannot be empty", code: :invalid_input}} =
               GetPendingGroupMembers.call("", session, credentials)
    end

    test "returns error when group_id is nil", %{session: session, credentials: credentials} do
      assert {:error, %ZcaEx.Error{message: "group_id cannot be empty", code: :invalid_input}} =
               GetPendingGroupMembers.call(nil, session, credentials)
    end
    ...
    test "raises error when service URL not found", %{session: session, credentials: credentials} do
      ...
      assert_raise RuntimeError, ~r/Service URL not found/, fn ->
        GetPendingGroupMembers.call("group123", session_no_service, credentials)
      end
    end
  end
end
