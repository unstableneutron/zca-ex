defmodule ZcaEx.Api.Endpoints.EditNoteTest do
  use ExUnit.Case, async: true

  alias ZcaEx.Api.Endpoints.EditNote
  ...
  describe "build_params/5" do
    test "builds correct params with pin? = false (pinAct = 2)", %{credentials: credentials} do
      ...
      assert params.params == ~s({"title":"Updated Note"})
  ...
  describe "build_url/1" do
    test "builds correct URL with session params", %{session: session} do
      url = EditNote.build_url(session)
    ...
    test "handles service URL as string", %{session: session} do
      ...
      assert url =~ "https://board2.zalo.me/api/board/topic/updatev2"
    end
  end
  ...
  describe "transform_note_detail/1" do
    test "parses params JSON string" do
      ...
      result = EditNote.transform_note_detail(data)
  ...
  describe "edit/6 validation" do
    test "returns error for empty group_id", %{session: session, credentials: credentials} do
      result = EditNote.edit("", "topic123", "Title", false, session, credentials)
    ...
    test "returns error for nil group_id", %{session: session, credentials: credentials} do
      result = EditNote.edit(nil, "topic123", "Title", false, session, credentials)
    ...
    test "returns error for non-string group_id", %{session: session, credentials: credentials} do
      result = EditNote.edit(123, "topic123", "Title", false, session, credentials)

      assert {:error, error} = result
      assert error.message == "group_id must be a non-empty string"
    end

    test "returns error for empty topic_id", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", "", "Title", false, session, credentials)

      assert {:error, error} = result
      assert error.message == "topic_id must be a non-empty string"
      assert error.code == :invalid_input
    end

    test "returns error for nil topic_id", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", nil, "Title", false, session, credentials)

      assert {:error, error} = result
      assert error.message == "topic_id must be a non-empty string"
    end

    test "returns error for non-string topic_id", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", 456, "Title", false, session, credentials)
    ...
    test "returns error for empty title", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", "topic123", "", false, session, credentials)
    ...
    test "returns error for nil title", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", "topic123", nil, false, session, credentials)
    ...
    test "returns error for non-string title", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", "topic123", 789, false, session, credentials)
    ...
    test "returns error for non-boolean pin?", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", "topic123", "Title", "yes", session, credentials)
    ...
    test "returns error for nil pin?", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", "topic123", "Title", nil, session, credentials)

      assert {:error, error} = result
      assert error.message == "pin? must be a boolean"
    end

    test "returns error for integer pin?", %{session: session, credentials: credentials} do
      result = EditNote.edit("group123", "topic123", "Title", 1, session, credentials)
    ...
    test "raises for missing service URL", %{session: session, credentials: credentials} do
      ...
      assert_raise RuntimeError, ~r/group_board service URL not found/, fn ->
        EditNote.edit("group123", "topic123", "Title", false, session_no_service, credentials)
      end
    end
  end
end
