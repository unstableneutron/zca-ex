defmodule ZcaEx.Api.Endpoints.AddGroupBlockedMemberTest do
  use ExUnit.Case, async: true

  alias ZcaEx.Api.Endpoints.AddGroupBlockedMember
  ...
  describe "normalize_member_ids/1" do
    test "converts single string to list" do
      assert AddGroupBlockedMember.normalize_member_ids("user123") == ["user123"]
    end

    test "keeps list as is" do
      assert AddGroupBlockedMember.normalize_member_ids(["user1", "user2"]) == ["user1", "user2"]
    end
  end

  describe "build_params/2" do
    test "builds correct params" do
      params = AddGroupBlockedMember.build_params("group123", ["user1", "user2"])
  ...
  describe "build_base_url/1" do
    test "builds correct base URL using group service", %{session: session} do
      url = AddGroupBlockedMember.build_base_url(session)

      assert url =~ "https://groupchat.zalo.me/api/group/blockedmems/add"
  ...
  describe "build_url/2" do
    test "builds URL with encrypted params", %{session: session} do
      ...
      url = AddGroupBlockedMember.build_url(session, encrypted)
  ...
  describe "call/4 validation" do
    test "returns error when group_id is empty", %{session: session, credentials: credentials} do
      assert {:error, %ZcaEx.Error{message: "group_id cannot be empty", code: :invalid_input}} =
               AddGroupBlockedMember.call("", "user123", session, credentials)
    end
    ...
    test "raises when group service URL not found", %{session: session, credentials: credentials} do
      ...
      assert_raise RuntimeError, ~r/Service URL not found for group/, fn ->
        AddGroupBlockedMember.call("group123", "user123", session_no_group, credentials)
      end
    end
  end
end
