defmodule ZcaEx.Api.Endpoints.RemoveFriendTest do
  use ExUnit.Case, async: true

  alias ZcaEx.Api.Endpoints.RemoveFriend
  ...
  describe "validate_user_id/1" do
    test "returns :ok for valid user_id" do
      assert :ok == RemoveFriend.validate_user_id("user123")
    end

    test "returns error for empty string" do
      assert {:error, %ZcaEx.Error{message: "user_id is required"}} =
               RemoveFriend.validate_user_id("")
    end

    test "returns error for nil" do
      assert {:error, %ZcaEx.Error{message: "user_id is required"}} =
               RemoveFriend.validate_user_id(nil)
    end

    test "returns error for non-string" do
      assert {:error, %ZcaEx.Error{message: "user_id must be a non-empty string"}} =
               RemoveFriend.validate_user_id(123)
    end
  end

  describe "build_params/2" do
    test "builds correct params" do
      params = RemoveFriend.build_params("user123", "test-imei")
  ...
  describe "build_base_url/1" do
    test "builds correct base URL", %{session: session} do
      url = RemoveFriend.build_base_url(session)

      assert url =~ "https://friend.zalo.me/api/friend/remove"
  ...
  describe "build_url/2" do
    test "builds URL with encrypted params", %{session: session} do
      ...
      url = RemoveFriend.build_url(session, encrypted)
  ...
  describe "call/3 validation" do
    ...
    test "raises when service URL not found", %{session: session, credentials: credentials} do
      ...
      assert_raise RuntimeError, ~r/Service URL not found/, fn ->
        RemoveFriend.call(session_no_service, credentials, "user123")
      end
    end
  end
end
