defmodule ZcaEx.Api.Endpoints.BlockUserTest do
  use ExUnit.Case, async: true

  alias ZcaEx.Api.Endpoints.BlockUser
  ...
  describe "build_base_url/1" do
    test "builds correct base URL", %{session: session} do
      url = BlockUser.build_base_url(session)

      assert url =~ "https://friend.zalo.me/api/friend/block"
  ...
  describe "build_url/2" do
    test "builds URL with encrypted params", %{session: session} do
      ...
      url = BlockUser.build_url(session, encrypted)
  ...
  describe "call/3 validation" do
    ...
    test "raises when service URL not found", %{session: session, credentials: credentials} do
      ...
      assert_raise RuntimeError, ~r/Service URL not found/, fn ->
        BlockUser.call(session_no_service, credentials, "user123")
      end
    end
  end
end
